...
def get_patent_claims(root_tree):
    ...
    def get_text_from_element(element):
        ...
        for child in element:
            ...
            if child_text:
                text_list.extend(child_text)
            if child.tail:
                text_list.append(child.tail.strip())
        return text_list

    for claim in claims:
        claim_data = {'id': claim.get('id'), 'num': claim.get('num')}
        claim_texts = claim.findall('.//claim-text')
        text_list = []

        for claim_text in claim_texts:
            text = get_text_from_element(claim_text)
            text_list.extend(text)

        # Concatenate the text elements into a single string
        claim_data['claim_text'] = ' '.join(text_list)
        claims_list.append(claim_data)
